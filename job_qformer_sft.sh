...
#SBATCH -o outputs/logs/qformer_sft-%j.out  # Output file name (%j = job ID)

# --- Optional Email Notifications ---
#SBATCH --mail-type=END,BEGIN,FAIL           # Send email on job END or FAIL
#SBATCH --mail-user=pvadhulas3@gatech.edu    # Email address
...
# Run SFT training with pre-trained Q-Former
echo "---------------------------------------"
echo "Running Q-Former SFT training (06_train_model.py with 27_train_mert_qformer config)..."
echo "---------------------------------------"
python scripts/06_train_model.py --config-name=27_train_mert_qformer
echo "---------------------------------------"
