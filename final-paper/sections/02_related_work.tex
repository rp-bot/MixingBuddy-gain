...
Early automatic mixing research centered on systems that captured expert knowledge through explicit mixing rules, including knowledge engineering and machine learning techniques for instrument-specific effects \cite{DeMan_Reiss_2013, bocko2010automatic, Chourdakis_Reiss_2017}.
...
\cite{waveunet} introduced Wave-U-Net autoencoders for automatic mixing.
\cite{Steinmetz_Pons_Pascual_Serra_2021} further advanced the field with a differentiable mixing console incorporating neural audio effects, enabling gradient-based optimization of mixing parameters.
These deep learning approaches achieved notable success in terms of accuracy, controllability, and diversity.
...
\cite{Venkatesh_Moffat_Miranda_2022} demonstrated word embeddings for automatic equalization in audio mixing. \cite{Chu_OReilly_Barnett_Pardo_2024} developed Text2FX, which harnesses CLAP \cite{clap} embeddings for text-guided audio effects.
...
\cite{Doh_Koo_Martinez-Ramirez_Liao_Nam_Mitsufuji_2024} investigated whether large language models can predict audio effects parameters from natural language, while \cite{Melechovsky_Mehrish_Herremans_2025} developed SonicMaster, a controllable all-in-one music restoration and mastering system.
...
One architectural approach, direct tokenization (also known as the unified approach), converts raw audio into discrete tokens and extends the LLM vocabulary to include these audio tokens.
...
The alignment is significatnly less resource-intensive than the unified approach.