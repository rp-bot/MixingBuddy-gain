...
\paragraph{Direction (Up/Down) Accuracy} We evaluate whether the \emph{sign} of the predicted differential gain (increase vs. decrease) matches the reference direction. We report direction accuracy and Matthews correlation (per-stem and overall). When references are intervals, direction is defined relative to the nearest point in the interval to the prediction.

\paragraph{Magnitude Metrics} We evaluate the accuracy of gain magnitude predictions, including absolute error and systematic bias in the predicted gain amounts.
